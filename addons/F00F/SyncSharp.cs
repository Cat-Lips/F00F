...
internal static partial class SyncSharp
{
    ...
    [GeneratedRegex(@"^application/bundle_identifier=""(?<Org>.+?)\.(?<PrjName>.+)\.(?<App>.+?)""", RegexOptions.ExplicitCapture | RegexOptions.Compiled)]
    private static partial Regex ExportBundleRegex();

    [GeneratedRegex(@"Godot_v(?<Version>\d+\.\d+)", RegexOptions.ExplicitCapture | RegexOptions.Compiled)]
    private static partial Regex LaunchSettingsRegex();
    ...
    public static void Execute()
    {
        var newName = GetName();
        var newDesc = GetDesc();
        var safeName = newName.Replace("'", "");
        var superSafeName = safeName.Replace(" ", "");

        UpdateReadme(newName, newDesc);
        UpdateProject(safeName);
        UpdateDataDir();
        RemoveOldFiles();
        UpdateWorkflows();
        UpdateExportConfig(superSafeName);
        ...
        static void UpdateReadme(string newName, string newDesc)
        {
            ...
            {
                var changed = false;
                UpdateLine(0, $"# {newName}", out var oldName);
                UpdateLine(1, newDesc, out var oldDesc);
                TrimTemplateText(oldName, oldDesc);
                return changed;

                void UpdateLine(int i, string newLine, out string oldLine)
                {
                    oldLine = null;
                    if (i < lines.Length)
                    {
                ...
                void TrimTemplateText(string oldName, string oldDesc)
                {
                    if (!changed) return;
                    if (oldName != "# F00F") return;
                    if (oldDesc != "Godot 4 C# Git Template") return;
                    ReportProgress($"Readme: Removing template text");
                    lines = lines[..2];
                }
            }
        }

        static void UpdateProject(string newName)
        {
            RenameProjectFiles();
            UpdateProjectFiles();
            ...
            void UpdateProjectFiles()
            {
                ...
                void UpdateCfg()
                {
                    ...
                    ReportProgress($"project.godot: {oldName} => {newName}");
        ...
        static void UpdateWorkflows()
        {
            ...
            bool UpdateLines(string source, string[] lines)
            {
                ...
                var changed = false;
                UpdateLine(2, "  GODOT_NAME:", $"{version}-{status}");
                return changed;
        ...
        static void UpdateExportConfig(string newName)
        {
        ...
        static void UpdateLaunchSettings()
        {
            const string source = "res://Properties/launchSettings.json";
            var lines = FS.ReadLines(source).ToArray();
            if (!UpdateLines()) return;
            FS.WriteLines(source, lines);
            ...
            {
                var changed = false;
                UpdateLines(GetGodotVersion(), LaunchSettingsRegex);
                return changed;

                void UpdateLines(string newVersion, Func<Regex> Regex)
                {
                    for (var i = 0; i < lines.Length; ++i)
                    {
                        var oldLine = lines[i];
                        var newLine = Regex().Replace(oldLine, newVersion);
                        if (oldLine == newLine) continue;

                        ReportProgress($"{source}: {oldLine.Trim()} => {newLine.Trim()} ({Regex().Match(oldLine).Value} => {newVersion})");
                        lines[i] = newLine;
                        changed = true;
                    }
                }
        ...
        static void RemovePluginDependencies()
        {
            const string Assets = "res://Assets";
            const string Plugin = "res://addons/F00F";
            const string RootIgnore = "res://.gitignore";
            const string AddonsIgnore = "res://addons/.gitignore";

            if (GetName() is "F00F") return;
            RemoveDependencies(Assets, Plugin);
            UpdateIgnoreFiles();

            void RemoveDependencies(params string[] resPaths)
            {
                foreach (var resPath in resPaths)
                    RemoveFolder(resPath);

                void RemoveFolder(string resPath)
                {
                    var sysPath = ProjectSettings.GlobalizePath(resPath);

                    if (FS.DirExists(sysPath) && !FS.IsSymLink(sysPath))
                    {
                        ReportProgress($"Removing Dependency: {sysPath}");
                        OS.MoveToTrash(sysPath);
                    }
                }
            }

            void UpdateIgnoreFiles()
            {
                IgnoreAssets(RootIgnore);
                IgnorePlugin(AddonsIgnore);

                void IgnoreAssets(string resPath)
                {
                    var sysPath = ProjectSettings.GlobalizePath(resPath);
                    var lines = FS.ReadLines(sysPath).ToArray();
                    if (!UpdateLines()) return;
                    FS.WriteLines(sysPath, lines);

                    bool UpdateLines()
                    {
                        if (lines.Length < 5) return false;
                        if (lines[4] != "#/Assets") return false;
                        ReportProgress($"Updating Ignore File: {sysPath}");
                        lines[4] = "/Assets"; return true;
                    }
                }

                void IgnorePlugin(string resPath)
                {
                    var sysPath = ProjectSettings.GlobalizePath(resPath);
                    var lines = FS.ReadLines(sysPath).ToArray();
                    if (!UpdateLines()) return;
                    ...
                    bool UpdateLines()
                    {
                        var myLines = lines.Where(x => x != string.Empty).ToArray();
                        if (myLines.Length is 2 && myLines.First() is "/*" && myLines.Last() is "!/F00F")
                        {
                            lines = ["/*"];
                            ReportProgress($"Updating Ignore File: {sysPath}");
                            return true;
    ...
    private static string GetGodotVersion()
    {
        ...
        return $"Godot_v{version["major"]}.{version["minor"]}";
    }
    ...
    private static class FS
    {
        ...
        public static bool IsSymLink(string path)
        {
            using (var dir = DirAccess.Open(path.GetBaseDir()))
                return dir.IsLink(path.GetFile());
        }
    }
