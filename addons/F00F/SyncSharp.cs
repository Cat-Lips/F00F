...
internal static partial class SyncSharp
{
    ...
    public static void Execute()
    {
        ...
        static void RemovePluginSourceCode()
        {
            const string Plugin = "res://addons/F00F";
            const string Ignore = "res://addons/.gitignore";
            ...
            {
                var folder = ProjectSettings.GlobalizePath(Plugin);

                if (FS.DirExists(folder) && !FS.IsSymLink(folder))
                {
                    ReportProgress($"Removing Plugin Folder: {folder}");
            ...
            {
                var source = ProjectSettings.GlobalizePath(Ignore);
                var lines = FS.ReadLines(source).ToArray();
                if (!UpdateLines()) return;
                FS.WriteLines(source, lines);
                ...
                bool UpdateLines()
                {
                    ...
                    {
                        lines = ["/*"];
                        ReportProgress($"Updating Ignore File: {source}");
                        return true;
                    }
